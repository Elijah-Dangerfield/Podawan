...
import com.dangerfield.features.login.internal.LoginViewModel.Action
import com.dangerfield.features.login.internal.LoginViewModel.Event
import com.dangerfield.features.login.internal.LoginViewModel.State
...
class LoginViewModel @Inject constructor(
    ...
    private suspend fun Action.UpdateEmail.updateEmail() {
        ...
        updateStateDebounced(duration = 1.seconds) {
            when {
                ...
                email.isValidEmail() -> it.copy(emailFieldState = Valid(email))
                else -> {
    ...
    private suspend fun Action.Login.login() {
        if (!state.isFormValid) {
            Timber.d("Form is not valid when submitting")
        } else {
